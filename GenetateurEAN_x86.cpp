// GenetateurEAN.cpp : Définit le point d'entrée de l'application.
//
...
int APIENTRY wWinMain( HINSTANCE hInstance,HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int       nCmdShow){
 ...
 //   LoadString(hInstance, IDC_GENETATEUREAN, szWindowClass, MAX_LOADSTRING);
    hInst = hInstance;
   ...
   HWND hDlg = CreateWindowEx(WS_EX_DLGMODALFRAME,szClassName, g_szWndTitle, WS_OVERLAPPED |  WS_CAPTION | WS_BORDER |WS_SYSMENU | WS_DLGFRAME,  CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, 0, 0, hInstance, 0);
   ...
   SendMessage(hCtrl, WM_SETFONT, (WPARAM)GetStockObject(18), 1L);
   hCtrl = CreateWindowEx(0, "Button", "Importer", WS_CHILD | WS_VISIBLE, rc.right - 335, rc.bottom - 25, 100, 22, hDlg, (HMENU)4000, wincl.hInstance, NULL);
   SendMessage(hCtrl, WM_SETFONT, (WPARAM)GetStockObject(18), 1L);
   hCtrl = CreateWindowEx(0, "Button", "A propos ...", WS_CHILD | WS_VISIBLE, rc.right - 447, rc.bottom - 25, 110, 22, hDlg, (HMENU)IDC_ABOUTHELP, wincl.hInstance, NULL);
   SendMessage(hCtrl, WM_SETFONT, (WPARAM)GetStockObject(18), 1L);
   GetLocalTime(&st);
   ...
   if (__argc > 1) {
	  ...
	  if (__argc =2) {
		  SetDlgItemText(hDlg,IDC_EDIT, __argv[1]);
	  }
	  if (__argc =3) {
		  SetDlgItemText(hDlg,IDC_EDIT, (char*) lpCmdLine[1]);
	  }
...
LRESULT CALLBACK WndProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam){
    ...
    case WM_COMMAND:        {
		  ...
		  case 4000:{
					char tmp[80];
					HWND hImport=FindWindow("#32770","Commis2Entrepot:Transformateurs | nouveau test Transformateur | v:2.1.1 | WindowsXP | 32bits");
					if(!hImport)return MessageBox(hDlg,"Interface non trouvee",wcex.lpszClassName,0);
					HWND hDesc=GetDlgItem(hImport,1042);
					HWND hDesc2=GetDlgItem(hDesc,6001);
					if(!hDesc) MessageBox(hDlg,"non connecte a : Administration",wcex.lpszClassName,0);
					GetWindowText(hDesc2,tmp,80);
					MessageBox(hDlg,tmp,0,0);
					}break;
          case IDC_BINARY:
